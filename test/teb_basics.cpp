...
#include <my_teb_local_planner/timed_elastic_band.h>

TEST(TEBBasic, autoResizeLargeValueAtEnd)
{
  double dt = 0.1;
  double dt_hysteresis = dt/3.;
  my_teb_local_planner::TimedElasticBand teb;
  
  teb.addPose(my_teb_local_planner::PoseSE2(0., 0., 0.));
  for (int i = 1; i < 10; ++i) {
    teb.addPoseAndTimeDiff(my_teb_local_planner::PoseSE2(i * 1., 0., 0.), dt);
  }
  // add a pose with a large timediff as the last one
  teb.addPoseAndTimeDiff(my_teb_local_planner::PoseSE2(10., 0., 0.), dt + 2*dt_hysteresis);
